...
// Prezzo "client-ready": ricarico fisso 30% (non modificabile in UI)
const CLIENT_MARKUP_PCT = 0.30;

function computeClientPrice(cost){
  // ricarico fisso
  return cost * (1 + CLIENT_MARKUP_PCT);
}
...
function updateClientPriceDisplay(){
  ...
  const price = computeClientPrice(LAST_COST);
